...
function Post() {
  ...
  const mark = markList.find((item) => post._id === item);

  function onSubmit() {
    createComment(
      { userRating: rating, comment, postId: post._id },
      {
        onSuccess: () =>
          setTimeout(() => {
  ...
  return (
    <div>
      <div>
        <PostImage defaultImage={defaultImage} imageCover={post.imageCover} styles="h-[600px] rounded mx-auto mb-10" />
      </div>
      ...
      <div className="py-3 px-40">
        <div className="flex justify-between items-center w-[97%] border-t border-neutral-700 pt-3">
          <div className="flex gap-3 items-center">
            ...
            <span className="text-base text-neutral-500">({Math.ceil(post.ratingAverage * 100) / 100})</span>
          </div>

          <FaBookmark
            onClick={() => updateMarkList(post._id)}
            className={`text-xl cursor-pointer ${mark ? "text-emerald-500" : "text-neutral-500"} hover:text-emerald-400 transition-all`}
          />
        </div>
        ...
        {email ? (
          role !== "admin" && !post.comments.find((comment) => comment.userId._id === id) ? (
            <div className="border border-emerald-700 rounded p-3 mt-16">
              <Form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="flex justify-between items-center">
                  <span className="text-neutral-500 ml-2">{COMMENT_LENGTH - comment.length} characters remains</span>
            ...
            <p className="text-xl text-emerald-600 mt-10">You already left a comment for this post</p>
          )
        ) : (
        ...
        {post.comments.length > 0 && (
          <Comment>
            ...
            <Comment.List comments={post.comments} render={(comment) => <Comment.Item comment={comment} key={comment._id} />} />
          </Comment>
        )}
      </div>
    </div>
